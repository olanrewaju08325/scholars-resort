...
import { CheckCircle, CheckCircle2, XCircle, ChevronLeft, ChevronRight, X, Bookmark, BookmarkPlus, Flag, AlertTriangle, Sparkles, MessageSquare, PauseCircle, PlayCircle, Clock, RotateCcw, Grid3X3, Layers, Camera, AlertCircle, Save, LogOut, BookOpen } from 'lucide-react';
            ...
            >
              ...
              <div className="space-y-3 md:space-y-4">
                ...
                {q.options.map((opt: string, i: number) => {
                  ...
                  if (isAnswered) {
                    const isCorrectOpt = checkIsCorrect(opt, q) || checkIsCorrect(optionLetter, q);
                    if (isCorrectOpt) {
                      btnClass = "bg-emerald-500/10 border-emerald-500 text-emerald-700 dark:text-emerald-300 font-semibold ring-1 ring-emerald-500/40";
                      Icon = <CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" />;
                      OptionTag = (
                        <span className="shrink-0 text-[10px] font-bold uppercase px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-600 dark:text-emerald-300 border border-emerald-500/30 flex items-center gap-1">
                          ✓ Correct Choice
                        </span>
                      );
                    } else if (opt === selectedAns) {
                      btnClass = "bg-rose-500/10 border-rose-500 text-rose-700 dark:text-rose-300 font-semibold ring-1 ring-rose-500/40";
                      Icon = <XCircle className="w-5 h-5 text-rose-500 shrink-0" />;
                      OptionTag = (
                        <span className="shrink-0 text-[10px] font-bold uppercase px-2 py-0.5 rounded bg-rose-500/20 text-rose-600 dark:text-rose-300 border border-rose-500/30 flex items-center gap-1">
                          ✗ Your Choice (Incorrect)
                        </span>
                      );
                    } else {
                      btnClass = "opacity-60 border-border bg-muted/20";
                      OptionTag = (
                        <span className="shrink-0 text-[10px] font-medium uppercase px-2 py-0.5 rounded bg-muted text-muted-foreground border border-border/50">
                          Incorrect
                        </span>
                      );
                    }
                  ...
                  return (
                    <div key={i} className="relative flex items-center w-full">
                      <Button 
                        variant="outline"
                        className={`w-full justify-start h-auto min-h-[3.25rem] py-3 pl-4 pr-4 text-left whitespace-normal text-sm sm:text-base font-normal transition-all rounded-xl gap-2 ${btnClass}`}
                        onClick={() => !isEliminated && handleSelect(opt)}
                        disabled={isAnswered || isPaused}
                      >
                        <span className="font-bold shrink-0 w-6 text-muted-foreground">
                          {optionLetter}.
                        </span>
              ...
              {isAnswered && (() => {
                ...
                return (
                  <div className="mt-8 pt-6 border-t border-border animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="bg-purple-500/5 border border-purple-500/20 rounded-xl p-4 md:p-5 mb-4 text-foreground leading-relaxed text-sm md:text-base space-y-4">
                      {isGeneratingAi ? (
                      ...
                      {/* Option-by-Option Reasoning Breakdown */}
                      {!isGeneratingAi && q.options && q.options.length > 0 && (
                        <div className="pt-3 border-t border-purple-500/20 space-y-2">
                          <h4 className="text-xs font-extrabold uppercase tracking-wider text-purple-600 dark:text-purple-400 flex items-center gap-1.5">
                            <BookOpen className="w-3.5 h-3.5" /> Option-by-Option Analysis & Reasoning
                          </h4>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                            {q.options.map((optItem: string, optIdx: number) => {
                              const letter = String.fromCharCode(65 + optIdx);
                              const isCorrectThis = checkIsCorrect(optItem, q) || checkIsCorrect(letter, q);
                              return (
                                <div
                                  key={optIdx}
                                  ...
                                  <div className="flex items-center justify-between gap-2 font-bold">
                                    <span className="flex items-center gap-1.5">
                                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-mono ${
                                        isCorrectThis ? 'bg-emerald-500 text-white' : 'bg-muted-foreground/20 text-muted-foreground'
                                      }`}>
                                        {letter}
                                      </span>
                                      <span className="truncate max-w-[120px]">{cleanOptionText(optItem)}</span>
                                    </span>
                                    <span className={`text-[9px] uppercase font-mono px-1.5 py-0.5 rounded ${
                                      isCorrectThis ? 'bg-emerald-500/20 text-emerald-600 dark:text-emerald-300' : 'bg-muted text-muted-foreground'
                                    }`}>
                                      {isCorrectThis ? 'Valid Choice' : 'Distractor'}
                                    </span>
                                  </div>
                                  <p className="text-[11px] leading-tight opacity-90">
                                    {isCorrectThis
                                      ? `Option ${letter} is the correct response as required by the syllabus specifications.`
                                      : `Option ${letter} is incorrect for this question construct.`}
                                  </p>
                                </div>
                              );
                            })}

...
// =======================================================
// --- AI SYNTHETIC QUESTION GENERATOR FOR CBT MOCKS ---
// =======================================================
app.post('/api/cbt/ai-mock-generate-questions', async (req, res) => {
  try {
    const { subject = 'Use of English', count = 5, weakTopics = [], difficulty = 'medium' } = req.body;
    const mockConfig = await getStoredSetting('ai_mock_config', DEFAULT_AI_MOCK_CONFIG);
    const customPrompt = mockConfig?.customPromptInstructions || 'Generate authentic JAMB UTME style multiple choice questions.';
    
    // Resolve Groq or Gemini key
    ...
    if (!groqKey) {
      try {
        const { data: dbKeys } = await supabase
          .from('admin_settings')
          .select('setting_key, setting_value')
          .in('setting_key', ['ai_api_keys', 'system_config']);
        if (dbKeys) {
          for (const row of dbKeys) {
            const val = row.setting_value?.apiKey || row.setting_value?.groq || row.setting_value?.groq?.apiKey;
    ...
    if (!groqKey) {
      return res.status(200).json({
        success: false,
        message: 'No AI key configured. Using database past question fallback.',
        questions: []
      });
    }

    const safeCount = Math.min(Math.max(1, Number(count) || 5), 15);
    const model = mockConfig?.aiModel || 'llama-3.3-70b-versatile';
    const weakContext = Array.isArray(weakTopics) && weakTopics.length > 0 
      ? `Focus specifically on these weak topics: ${weakTopics.slice(0, 3).join(', ')}.` 
      : '';

    const systemPrompt = `You are the Official JAMB UTME Exam Question Synthesizer.
Generate exactly ${safeCount} authentic Nigerian UTME (JAMB) multiple-choice questions for the subject "${subject}".
${weakContext}
Difficulty target: ${difficulty}.
${customPrompt}

IMPORTANT: Respond ONLY with a valid JSON array of objects. Do not include markdown formatting or backticks.
...
[
  {
    ...
    "correct_option": "A",
    "explanation": "Detailed explanation of why this option is correct based on syllabus.",
    "topic_name": "Specific syllabus topic"
  }
]`;
    ...
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model,
        messages: [
          { role: 'system', content: 'You are an expert exam question generator for JAMB UTME. You output only raw valid JSON arrays.' },
          { role: 'user', content: systemPrompt }
        ],
        temperature: mockConfig?.temperature ?? 0.3,
        max_tokens: Math.min(3500, (safeCount * 250) + 500)
      })
    });

    if (!response.ok) {
      const errText = await response.text();
      console.warn('AI Mock Question Generation API error:', errText);
      return res.status(200).json({ success: false, questions: [] });
    }
    ...
    const formatted = parsedQuestions.map((q: any, idx: number) => ({
      id: `ai_synth_${Date.now()}_${idx}_${Math.random().toString(36).substring(2, 6)}`,
      question_text: q.question_text || q.question || '',
      question: q.question_text || q.question || '',
      options: Array.isArray(q.options) ? q.options : [],
      correct_answer: (q.correct_option || q.correct_answer || 'A').toUpperCase().trim(),
      correct_option: (q.correct_option || q.correct_answer || 'A').toUpperCase().trim(),
      explanation: q.explanation || 'Authentic UTME question rationale generated by AI.',
      subject_name: subject,
      topic_name: q.topic_name || `${subject} Core`,
      year: 2025,
      difficulty: difficulty,
      is_ai_generated: true
    })).filter((q: any) => q.question_text && q.options.length >= 4);
    ...
    console.error('AI Mock generation failure:', err);
    return res.status(200).json({ success: false, questions: [], error: err.message });
  }
});

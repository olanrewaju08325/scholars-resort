...
const MASTER_ADMIN_EMAILS = ['admitwise2@gmail.com', 'olanrewajuhamilot@gmail.com'];
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (error) {
      console.warn(`[Profile DB Error]`, error.message);
    }

    const email = dbProf?.email?.toLowerCase().trim();
    const isMaster = email && MASTER_ADMIN_EMAILS.includes(email);

    const profile = {
      ...(dbProf || { id: userId }),
      role: isMaster ? 'admin' : (dbProf?.role || 'student'),
      has_paid: isMaster ? true : !!dbProf?.has_paid,
      onboarding_completed: isMaster ? true : !!dbProf?.onboarding_completed
    };
